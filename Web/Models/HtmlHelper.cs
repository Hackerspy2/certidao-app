...
public class HtmlHelper
{
        ...
        string headerColunas, string tituloBtn = null)
    {
        ...
        if (!string.IsNullOrWhiteSpace(action))
            ...
            if (!string.IsNullOrWhiteSpace(tituloBtn))
            {
                sb.AppendFormat(
                    action.Contains("?")
                        ? "<a href='/{0}/{1}&tipo=add' class='{2} waves-effect waves-light btn-inverse btn btn-sm pull-right m-l-10' data-toggle='tooltip' title='{3}' data-placement='right'>{4}</a>"
                        : "<a href='/{0}/{1}/?tipo=add' class='{2} waves-effect waves-light btn-inverse btn btn-sm pull-right m-l-10' data-toggle='tooltip' title='{3}' data-placement='right'>{4}</a>",
        ...
        sb.Append(
            "<table id='dynamicTableDesc' class='display nowrap table table-hover table-striped table-bordered' cellspacing='0' width='100%'>");
        sb.Append("<thead><tr>");
        ...
        string headerColunas, string tituloBtn = null)
    {
        ...
        sb.Append(
            "</div><div class='animated fadeInUp'><div class='row'><div class='col-12'><div class='card'><div class='card-body'><div class='table-responsive'>");
        sb.Append(
            "<table id='dynamicTableDesc' class='display nowrap table table-hover table-striped table-bordered'>");
        sb.Append("<thead><tr>");
        foreach (var c in headerColunas.Split('|')) sb.AppendFormat("<th>{0}</th>", c);
        sb.Append("</tr></thead><tbody></tbody></table></div></div></div></div></div></div>");
        return sb.ToString();
        ...
        string headerColunas, string classPaiModal, string inputUpload = null)
    {
        ...
        sb.AppendFormat(
            "<table id='{0}' class='display nowrap table table-hover table-striped table-bordered'>",
            idTabela);
        sb.Append("<thead><tr>");
        foreach (var c in headerColunas.Split('|')) sb.AppendFormat("<th>{0}</th>", c);
        ...
        string headerColunas, string classPaiModal)
    {
        ...
        sb.Append("<div class='table-responsive'>");
        sb.AppendFormat(
            "<table id='{0}' class='display nowrap table table-hover table-striped table-bordered' cellspacing='0' width='100%'>",
            idTabela);
        sb.Append("<thead><tr>");
        foreach (var c in headerColunas.Split('|')) sb.AppendFormat("<th>{0}</th>", c);
