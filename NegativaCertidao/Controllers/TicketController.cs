...
public class TicketController : ApplicationController<Ticket>
{
    ...
    public async Task<JsonResult> ConsultaPagamento(string txid)
    {
        ...
        //await _services.GerarToken(_configuration.GetSection("LyTex.ClientId").Value,
        //    _configuration.GetSection("LyTex.ClientSecret").Value,
        //    new[] { "invoice", "client", "subscription", "paymentLink", "installment", "product", "billingRule" });

        //var consultaCliente = await _services.ConsultarCobranca(txid);
    ...
    public async Task<JsonResult> ContatoEnviar(string nome, string celular, string email, string mensagem)
    {
        try
        {
            var retornoajax = Request.Form["g-recaptcha-response"];
            ...
            if (string.IsNullOrWhiteSpace(retornoajax)) return Json(new { cssClass = "error", mensagem = "Resolva o captcha!" });
            if (!VerificaCaptcha(retornoajax)) return Json(new { cssClass = "error", mensagem = "Falha na verificação captcha!" });
