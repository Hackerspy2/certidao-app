...
var carregarDadosCallFuncion = function (campo, url, callFunction) {
    ...
    $(campo).typeahead({ hint: true, highlight: true, minLength: 1 }, {
        ...
        source: dados,
        templates: {
            empty: [`<div class='empty-message'><a href='${urlAdd}' class="detalhesPaiPai hand" data-orginal-title='${title}' title='${title}'>Nenhuma informação encontrado, clique aqui e faça o cadastro</a></div>`].join('\n'),
            suggestion: function (data) { return '<p>' + data.nome + '</p>'; }
        }
    }).bind('typeahead:select', function (node, item) {
        node.val(item.Nome);
        callFunction();
    });
};
    ...
    $.fn.addItems = function (data, id) {
        return this.each(function () {
            ...
            $.each(data, function (index, itemData) {
                var option = new Option(itemData.text, itemData.value, itemData.value.toString() == id ? true : false, itemData.value.toString() == id ? true : false);
                list.add(option);
            });
        });
    };
...
var removeImagem = function (controller, id) {
    $("#removerFoto").click(function (e) {
        e.preventDefault();
        var link = $(this);
        var modal = window.parent.$('#modalPaiConfirm').one('click', '#btnPaiConfirme', function () {
            $.ajax({
...
var carregarEstados = function (inputEstado, inputCidade, valorEstado, valorCidade) {
    $(function () {
        $.getJSON('/admin/estados_cidades.json', function (data) {
            var items = [];
            var options = '<option value=""></option>';
            $.each(data, function (key, val) {
                var select = valorEstado == val.sigla ? "selected=selected" : "";
                ...
                $.each(data, function (key, val) {
                    if (val.nome == str) {
                        $.each(val.cidades, function (key_city, val_city) {
                            var select = valorCidade == val_city ? "selected=selected" : "";
...
var WinModal = function (controle) {
    $(function () {
        $(document).delegate(controle, 'click', function (e) {
            e.preventDefault();
            var c = $(this);
            var modal = $('#modalAdm');
...
var WinModalParentParent = function (controle, funcao) {
    $(function () {
        $(document).delegate(controle, 'click', function (e) {
            e.preventDefault();
            var c = $(this);
            var modal = window.parent.$('#modalPaiPaiAdm');
...
var WinModalParentParentParent = function (controle, funcao) {
    $(function () {
        $(document).delegate(controle, 'click', function (e) {
            e.preventDefault();
            var c = $(this);
            var modal = window.parent.parent.$('#modalPaiPaiPaiAdm');
...
var jDataTableAll = function (idTabela, url, controller, rowConfirm, classDetails, classEdit, classDel, janela = "", order = "", searching = true) {
...
var jDataTable = function (idTabela, url, controller, rowConfirm, classDetails, classEdit, classDel, janela = "", order = "") {
    myTable = $(idTabela).dataTable({
        order: [[rowConfirm, order != "" ? order : "asc"]],
        processing: true,
        sAjaxSource: url,
        ...
        aoColumnDefs: [
            ...
            { bSearchable: false, aTargets: [-1] },
            {
                mRender: function (data, type, row) {
                    ...
                    if (classEdit != "hide-item") {
                        html += "<a href='/" + controller + "/Edit/?id=" + id + "' title='Alterar' data-original-title='Alterar - " + row[rowConfirm] + "' class='alterar" + janela + " " + classEdit + " dropdown-item'> Alterar</a>";
                    }
                    if (classDel != "hide-item") {
                        html += "<a href='javascript:;' title='Deletar' class='deletar" + janela + " " + classDel + " dropdown-item' data-rel='" + id + "|" + row[rowConfirm] + "|" + controller + "|Del'> Deletar</a>";
                    }
...
var jDataTableLinkExtra = function (idTabela, url, controller, rowConfirm, classDetails, classEdit, classDel, janela = "", htmlExtra = "", order = "") {
    myTable = $(idTabela).dataTable({
...
var jDataTableNoAlterouLinkExtra = function (idTabela, url, controller, rowConfirm, classDetails, classEdit, classDel, janela = "", htmlExtra = "", order = "") {
    myTable = $(idTabela).dataTable({
        order: [[rowConfirm, order != "" ? order : "desc"]],
        sAjaxSource: url,
        bServerSide: true,
...
var jDataTableNoSearch = function (idTabela, url, controller, rowConfirm, classDetails, classEdit, classDel, janela = "") {
    myTable = $(idTabela).dataTable({
        sAjaxSource: url,
        bServerSide: true,
        searching: false,
        ...
        aoColumnDefs: [
            ...
            { bSearchable: false, aTargets: [-1] },
            {
                mRender: function (data, type, row) {
                    ...
                    if (classEdit != "hide-item") {
                        html += "<a href='/" + controller + "/Add/?tipo=update&janela=bisneta&id=" + id + "' title='Alterar' data-original-title='Alterar - " + row[rowConfirm] + "' class='alterar" + janela + " " + classEdit + " dropdown-item'> Alterar</a>";
                    }
                    if (classDel != "hide-item") {
                        html += "<a href='javascript:;' title='Deletar' class='deletar" + janela + " " + classDel + " dropdown-item' data-rel='" + id + "|" + row[rowConfirm] + "|" + controller + "|Del'> Deletar</a>";
                    }
...
var jDataTableNoSearchAndNoAction = function (idTabela, url, controller, rowConfirm, classDetails, classEdit, classDel, janela = "") {
    myTable = $(idTabela).dataTable({
        sAjaxSource: url,
        bServerSide: true,
        searching: false,
        ...
        initComplete: function () {
            var coluna = $(idTabela + ' th').length - 1;
        ...
        aoColumnDefs: [
            ...
            { bSearchable: false, aTargets: [-1] },
            {
                mRender: function (data, type, row) {
                    ...
                    html += "<a href='javascript:;' title='Deletar' class='deletarPai " + janela + " " + classDel + " dropdown-item' data-rel='" + id + "|" + row[rowConfirm] + "|" + controller + "|Del'> Deletar</a>";
                    html += "</div></div>";
                    return html;
                },
                aTargets: [-1]
...
var jDataTableEditDel = function (idTabela, url, orderColum, orderType, controller, rowConfirm, classDel, classEdit, janela = "") {
    myTable = $(idTabela).dataTable({
        sAjaxSource: url,
        order: [[orderColum, orderType]],
        bServerSide: true,
...
var jDataTableVisuDel = function (idTabela, url, orderColum, orderType, controller, rowConfirm, classDel, classDetails, janela = "") {
    myTable = $(idTabela).dataTable({
        sAjaxSource: url,
        order: [[orderColum, orderType]],
        bServerSide: true,
        ...
        aoColumnDefs: [
            ...
            { bSearchable: false, aTargets: [-1] },
            {
                mRender: function (data, type, row) {
                    ...
                    html +=
                        "<a href='javascript:void(0)' class='icon-options-vertical link' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'></a><div class='dropdown-menu dropdown-menu-right'>";
                    html += "<a href='/" + controller + "/Baixar/?id=" + id + "' title='Visualizar' data-original-title='Visualizar - " + row[rowConfirm] + "' class='" + classDetails + " dropdown-item'> Baixar</a>";
                    html += "<a href='javascript:;' title='Deletar' class='deletar" + janela + " " + classDel + " dropdown-item' data-rel='" + id + "|" + row[rowConfirm] + "|" + controller + "|Del'> Deletar</a>";
                    html += "</div></div>";
                    return html;
                },
...
var jDataTableDownFixo = function (idTabela, url, controller, rowConfirm, classDetails, classDel, rowArquivo, janela = "") {
    myTable = $(idTabela).dataTable({
        sAjaxSource: url,
        bServerSide: true,
        bDestroy: true,
        ...
        aoColumnDefs: [
            ...
            { bSearchable: false, aTargets: [-1] },
            {
                mRender: function (data, type, row) {
                    ...
                    html +=
                        "<a href='javascript:void(0)' class='icon-options-vertical link' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'></a><div class='dropdown-menu dropdown-menu-right'>";
                    html += "<a href='/images/" + controller + "/" + row[rowArquivo] + "' title='Visualizar' target='_blank' data-original-title='Visualizar - " + row[rowConfirm] + "' class='" + classDetails + " dropdown-item'> Visualizar</a>";
                    html += "<a href='javascript:;' title='Deletar' class='deletar" + janela + " " + classDel + " dropdown-item' data-rel='" + id + "|" + row[rowConfirm] + "|" + controller + "|Del'> Deletar</a>";
                    html += "</div></div>";
                    return html;
                },
                ...
                mRender: function (data, type, row) {
                    var id = data;
                    var html = '<input type="text" name="Descricao" id="Descricao" value="' + (row[1] == null ? "" : row[1]) + '" class="form-control set-descricao" placeholder="Informe uma descrição" data-id=' + row[3] + ' data-validation-required-message="obrigatório" aria-invalid="false" />';
                    return html;
                },
                aTargets: [1]
            }
...
$(function () {
    ...
    $(document).delegate(".hand", "click", function () {
    ...
    $(document).delegate(".changeStatusAss", "click", function (e) {
        e.preventDefault();
        let id = $(this).attr("data-id");
        const modal = window.parent.$('#modalPaiConfirm').one('click', '#btnPaiConfirme',
            function () {
                window.parent.$(".preloader").show();
                $.ajax({
    ...
    $(document).delegate(".so-numeros", "keyup", function () {
        var valor = $(this).val().replace(/[^0-9]+/g, '');
        $(this).val(valor);
    });

    $(document).delegate(".so-letras", "keyup", function () {
    ...
    $("body").delegate(".deletar", 'click', function (e) {
        ...
        var modal = $('#modalConfirm').one('click', '#btnConfirme', function () {
            $.ajax({
                url: "/" + dados[2] + "/" + dados[3] + "/",
                type: "get",
                data: { id: dados[0] },
    ...
    $("body").delegate(".deletarPai", 'click', function (e) {
        e.preventDefault();
        var link = $(this);
        var dados = link.attr("data-rel").split("|");
        var modal = window.parent.$('#modalPaiConfirm').one('click', '#btnPaiConfirme', function () {
    ...
    $("body").delegate(".deletarPaiPai", 'click', function (e) {
        e.preventDefault();
        var link = $(this);
        var dados = link.attr("data-rel").split("|");
        var modal = window.parent.parent.$('#modalPaiPaiConfirm').one('click', '#btnPaiPaiConfirme', function () {
            $.ajax({
                url: "/" + dados[2] + "/" + dados[3] + "/",
                type: "get",
                data: { id: dados[0] },
                contentType: 'application/json',
                success: function (result) {
                    window.parent.parent.NotificaClose(getTitle(result.cssClass), result.mensagem, result.cssClass);
                    if (result.cssClass === "success") {
                        link.parents().find("#" + dados[0]).remove();
                    }
                }
    ...
    $("body").delegate(".deletarDetalhes", 'click', function (e) {
        e.preventDefault();
        var link = $(this);
        var dados = link.attr("data-rel").split("|");
        var modal = $('#modalPaiConfirm').one('click', '#btnPaiConfirme', function () {
            $.ajax({
                url: "/" + dados[2] + "/" + dados[3] + "/",
                type: "get",
                data: { id: dados[0] },
